...
if(!dojo._hasResource["dair.Console"]){ //_hasResource checks added by build. Do not use _hasResource directly in your code.
...
(function(){
	...
	if(air.Introspector !== undefined){
		...
		hasConsole = false;
		if(userConfig.file){
			var _file = new dair.Log(userConfig.options);
			_out = function(){
				_file.write.apply(_file, arguments);
			}
		}else{
			_out = air.trace;
		}
		_c = {
			...
			log: function(){
				_out(this.argsToString.apply(this, arguments));
			},
			debug: function(){
				_out(this.argsToString.apply(this, arguments));
			},
			info: function(){
				_out("[I] "+this.argsToString.apply(this, arguments));	
			},
			warn: function(){
				_out("[WARN] "+this.argsToString.apply(this, arguments));	
			},
			error: function(){
				_out("[ERROR] "+this.argsToString.apply(this, arguments));	
			},
			dump: function(obj){
				_out(printObject(obj));	
			}
		};
	...
	window.console = {
		...
		dir: function(obj){
			_c.dump.call(_c, printObject(obj));
		},
		dirxml: function(obj){
			_c.dump.call(_c, printObject(obj));
		},
	...
	var printObject = function(o, i, txt, used){
		...
		for(var nm in o){
			...
			}else if(typeof(o[nm]) == "object" && o[nm] && typeof o[nm] != "function"){
	...
	if (window.outputQueue) {
		// the queued logs that are cached in AIR.js
		window.outputQueue();
	}
})();
