 ...
 //construct the button's html
  ...
  jQuery(wym._box).find('li.wym_tools_fullscreen a').click(function() {
    if (jQuery(wym._box).css('position') != 'fixed') {
      ...
      jQuery('#loader').css({'position' : 'fixed', 'background-color': 'rgb(0, 0, 0)', 'opacity': '0.8', 'z-index': '99999', 'width': '100%', 'height': '100%', 'top': '0px', 'left': '0px'});
      jQuery(wym._box).css({'position' : 'fixed', 'z-index' : '99', 'top': '5%', 'left': '5%', 'width': '90%', 'height': '90%'});
	  jQuery(".wym_iframe").addClass("fsexpand");
    } else {
      jQuery('#loader').remove();
	  jQuery(".wym_iframe").removeClass("fsexpand");
