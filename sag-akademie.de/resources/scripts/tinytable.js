...
TINY.table = function() {
	...
	sorter.prototype.wk = function(y, asc) {
		var t = ge(this.e), i = 0; // x=t.h.cells[y],
		for (i; i < this.l; i++) {
			t.a[i].o = i;
			var v = t.r[i].cells[y];
			//t.r[i].style.display = '';
		...
		for (i = 0; i < this.l; i++) {
			...
			n.appendChild(r);
			r.className = r.className + (i % 2 == 0 ? this.even : this.odd);
			var cells = T$$('td', r);
			for (var z = 0; z < t.w; z++) {
	...
	function cpAsc(f, c) {
		...
		var i = readISO8601date(f);
		var n = readISO8601date(c);
		
		if (!isNaN(i) && !isNaN(n)) { //if is date
			g = i;
			h = n;
		} else { // no date?! perhaps a float or int 
	...
	function readISO8601date(Q) { 
		...
		if ((T = /^(\d+).(\d+).(\d+)$/.exec(Q)) == null) {
			if ((T = /^(\d+).(\d+).(\d+) (\d+):(\d+):(\d+)$/.exec(Q)) == null) {
				return NaN;
			} else {
				T.pop(); // throw away seconds
				min = parseInt(T.pop(),10);
				hour =  parseInt(T.pop(),10);
			}
		} 
		
		
		var year = parseInt(T.pop(), 10);
		var month = parseInt(T.pop(), 10);
		var day = parseInt(T.pop(), 10);	
			
		var cmp = year * 1000000000000 + month * 1000000 + day * 10000 + hour * 100 + min;
