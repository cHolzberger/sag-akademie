...
class BuchungNotificationHandler extends GenericEventHandler {
	...
	function sendPublicNewEmail($uuidArr) {
		...
		qlog("Finding Buchungen");
		foreach ($uuidArr as $uuid) {
			$buchung = Doctrine::getTable("ViewBuchungPreis")->detailedByUuid()->execute($uuid, Doctrine::HYDRATE_ARRAY);
			$buchung = $buchung[0];
			array_push($buchungen, $buchung);
		}
		qlog("Creating EMail template");
		$email = new MosaikEmail();
		$email->setContainer("__engine");
		...
		$email->addData("Kontakt", $buchung['Person']['Kontakt']);
		$email->addData("link", MosaikConfig::getVar("webUrl") . "kunde/buchung/$uuid");
		$email->addData("Uuid", $uuid);
		...
		if ( is_object ( $ansprechpartner ) && !empty($ansprechpartner->email)) {
			...
			try {
				...
				$tmp = new Person();			
				$email->addData("Ansprechpartner", $tmp->toArray(true));
	...
	function sendPublicEmail($uuid, $oldId) { // nur geänderte daten
		...
		$barr[0]['AlterStandort'] =  $alteBuchung->Seminar->Standort->name;
		...
		$email->addData("link", MosaikConfig::getVar("webUrl") . "kunde/buchung/$uuid");
		$email->addData("Uuid", $uuid);
		$email->addData("AlteBuchung", $alteBuchung->toArray(true));
		$email->addData("AlterStandort", $alteBuchung->Seminar->Standort->name);
	 ...
	 */
	function sendChangeMail($id,$format="id") {
		$email = new MosaikEmail();
		$email->setContainer("__engine");
		$email->setPage("buchung_bestandskunde");
