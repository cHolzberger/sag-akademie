...
AND buchung.deleted_at IS NULL;
...
WHERE hotel_buchung.deleted_at IS NULL;
...
( 1-(100 - buchung.rabatt) / 100 ) as rabattInProzent,
 ...
 (CASE
  ...
  WHEN buchung.umbuchungs_datum IS NOT NULL THEN 3
...
AND buchung.deleted_at IS NOT NULL;
...
WHERE akquise_kontakt.quelle_id = kontakt_quelle.id AND akquise_kontakt.deleted_at IS NULL
...
akquise_kontakt.quelle_id = kontakt_quelle.id AND akquise_kontakt.deleted_at IS NULL
