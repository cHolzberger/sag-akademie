...
ALTER TABLE `buchung` 
CHANGE COLUMN `bildungscheck` `bildungscheck` TINYINT(1) NULL ,
CHANGE COLUMN `arbeitsagentur` `arbeitsagentur` TINYINT(1) NULL ,
CHANGE COLUMN `bestaetigt` `bestaetigt` TINYINT(1) NULL DEFAULT 0 ,
CHANGE COLUMN `gebucht_ueber` `gebucht_ueber` VARCHAR(255) NULL DEFAULT '' ,
CHANGE COLUMN `bemerkung` `bemerkung` TEXT NULL ,
CHANGE COLUMN `preis_alt` `preis_alt` DOUBLE NULL ,
CHANGE COLUMN `notiz` `notiz` TEXT NULL ,
CHANGE COLUMN `bildungscheck_ausstellung_datum` `bildungscheck_ausstellung_datum` VARCHAR(255) NULL DEFAULT '' ,
CHANGE COLUMN `rabatt` `rabatt` INT(11) NULL DEFAULT 0 ,
CHANGE COLUMN `vdrk_referrer` `vdrk_referrer` INT(11) NULL DEFAULT '0' ,
CHANGE COLUMN `paketbuchung` `paketbuchung` INT(11) NULL DEFAULT '0' ,
CHANGE COLUMN `arbeitsagenturanteil` `arbeitsagenturanteil` DOUBLE NULL ,
CHANGE COLUMN `rechnungsbetrag` `rechnungsbetrag` DOUBLE NULL ,
CHANGE COLUMN `gutschrift` `gutschrift` DOUBLE NULL ,
CHANGE COLUMN `anteilvdrk` `anteilvdrk` DOUBLE NOT NULL DEFAULT 0 ,
CHANGE COLUMN `nettoep` `nettoep` DOUBLE NOT NULL DEFAULT 0 ,
CHANGE COLUMN `kursgebuehr` `kursgebuehr` DOUBLE NOT NULL DEFAULT 0 ,
CHANGE COLUMN `versorgungsamtanteil` `versorgungsamtanteil` DOUBLE NULL DEFAULT 0 ,
CHANGE COLUMN `import` `import` INT(1) NULL DEFAULT 0 ,
CHANGE COLUMN `info_kunde` `info_kunde` TEXT NULL ,
CHANGE COLUMN `bildungscheck_ausstellung_bundesland_id` `bildungscheck_ausstellung_bundesland_id` INT(11) NULL ,
CHANGE COLUMN `import_notiz` `import_notiz` TEXT NULL ,
CHANGE COLUMN `nicht_importierfaehig` `nicht_importierfaehig` TINYTEXT NULL ,
CHANGE COLUMN `rechnungsnummer` `rechnungsnummer` VARCHAR(255) NULL ,
CHANGE COLUMN `teilgenommen` `teilgenommen` INT(11) NULL ,
CHANGE COLUMN `rechnungsnummer1` `rechnungsnummer1` VARCHAR(255) NULL ,
CHANGE COLUMN `rechnungsnummer2` `rechnungsnummer2` VARCHAR(255) NULL ,
CHANGE COLUMN `bildungscheck_ausstellung_art` `bildungscheck_ausstellung_art` VARCHAR(255) NULL ,
CHANGE COLUMN `alte_buchung` `alte_buchung` INT(11) NULL COMMENT 'wird gesetzt wenn diese buchung aus einer alten umgebucht wird, zeigt auf die urspruengliche buchung' ,
CHANGE COLUMN `version` `version` INT(11) NULL,
CHANGE COLUMN `storno_status` `storno_status` VARCHAR(255) NULL DEFAULT '',
CHANGE COLUMN `zustaendige_arbeitsagentur` `zustaendige_arbeitsagentur` VARCHAR(255) NULL DEFAULT '' ,
CHANGE COLUMN `bestaetigt_homepage` `bestaetigt_homepage` VARCHAR(255) NULL DEFAULT '' ,
CHANGE COLUMN `bildungscheck_nrw` `bildungscheck_nrw` VARCHAR(255) NULL DEFAULT '' ,
CHANGE COLUMN `bildungsgutschein_hessen` `bildungsgutschein_hessen` VARCHAR(255) NULL DEFAULT '' ;


ALTER TABLE `sag-akademie_de_stable`.`buchung_alt` 
ADD COLUMN `gebucht_ueber` VARCHAR(255) NULL AFTER `umgebuchtID`;


ALTER TABLE `buchung_alt` 
CHANGE COLUMN `bildungscheck` `bildungscheck` TINYINT(1) NULL ,
CHANGE COLUMN `arbeitsagentur` `arbeitsagentur` TINYINT(1) NULL ,
CHANGE COLUMN `bestaetigt` `bestaetigt` TINYINT(1) NULL DEFAULT 0 ,
CHANGE COLUMN `gebucht_ueber` `gebucht_ueber` VARCHAR(255) NULL DEFAULT '' ,
...
CREATE OR REPLACE VIEW view_buchung AS
	SELECT
		...
		(CASE
		 WHEN buchung.storno_datum IS NOT NULL THEN 2
		 WHEN buchung.teilgenommen IS NOT NULL THEN 5
		 WHEN buchung.umgebucht_id != '0' THEN 3
		 WHEN seminar.storno_datum IS NOT NULL THEN 4
...
CREATE OR REPLACE VIEW view_buchung_preis AS
	...
	WHERE buchung.deleted_at IS NULL;
...
CREATE OR REPLACE ALGORITHM = MERGE VIEW view_buchungen_stats AS
	...
	WHERE seminar.storno_datum IS NULL
	      AND buchung.storno_datum IS NULL
...
--  kontakte mit buchungen
 ...
 select 
 `buchung`.`uuid` AS `id`,`buchung`.`id` AS `buchung_id`,`person`.`kontakt_id` AS `kontakt_id`,`person`.`id` AS `person_id`,`kontakt`.`firma` AS `firma`,`kontakt`.`strasse` AS `firma_strasse`,`kontakt`.`ort` AS `firma_ort`,`kontakt`.`plz` AS `firma_plz`,`buchung`.`datum` AS `buchungsdatum`,`seminar`.`seminar_art_id` AS `seminar_art_id`,`seminar`.`datum_begin` AS `seminar_datum`,`seminar`.`kursnr` AS `kursnr`,(case when (`buchung`.`storno_datum` IS NOT NULL) then 2 when (`buchung`.`teilgenommen` IS NOT NULL) then 5 when (`buchung`.`umgebucht_id` <> '0') then 3 when (`seminar`.`storno_datum` IS NOT NULL) then 4 when (`buchung`.`bestaetigt` = 1) then 1 else 1 end) AS `buchung_status` from (((`person` join `kontakt`) join `buchung`) join `seminar`) where ((`buchung`.`deleted_at` IS NOT NULL) and (`buchung`.`person_id` = `person`.`id`) and (`kontakt`.`id` = `person`.`kontakt_id`) and (`buchung`.`seminar_id` = `seminar`.`id`));
